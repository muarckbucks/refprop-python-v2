...
def _sat_curve(
    ...
    """
    Calcula la curva de saturación (líquido + vapor) e incluye el punto
    crítico como último punto de ambas ramas para garantizar el cierre.

    Devuelve s_liq, T_liq, s_vap, T_vap, s_crit, T_crit (arrays numpy + escalares).
    ...
    # Barrido en dos fases: resolución normal hasta cerca del crítico,
    # luego pasos muy finos para aproximarse tanto como sea posible al vértice
    # donde s_liq y s_vap convergen (esto funciona igual para puros y mezclas).
    temps_coarse = np.linspace(T_min_C, T_crit_C - 1.0, n_points)
    temps_fine   = np.linspace(T_crit_C - 1.0, T_crit_C - 1e-4, max(n_points // 4, 50))
    temps        = np.unique(np.concatenate([temps_coarse, temps_fine]))
    ...
    # El vértice de cierre es la media de los últimos puntos calculados de
    # cada rama: en el límite ambos convergen al mismo valor, pero usando la
    # media se elimina cualquier pequeña asimetría numérica de REFPROP.
    if s_liq and s_vap:
        s_crit = (s_liq[-1] + s_vap[-1]) / 2.0
        T_top  = (T_liq[-1] + T_vap[-1]) / 2.0
        # Reemplazar el último punto de ambas ramas por el vértice único
        s_liq[-1] = s_crit;  T_liq[-1] = T_top
        s_vap[-1] = s_crit;  T_vap[-1] = T_top
    else:
        s_crit = float("nan")
        T_top  = T_crit_C
...
def plot_ts_diagrams(
    ...
    *,
    ...
    for s in series:
        ...
        try:
            s_liq, T_liq, s_vap, T_vap, s_crit, T_crit_C = _sat_curve(
        ...
        # Dibujar rama líquida hasta el punto crítico (ya incluido al final)
        ax.plot(s_liq, T_liq, color=s.color, linewidth=s.linewidth, linestyle=s.linestyle)
        # Dibujar rama vapor hasta el punto crítico (ya incluido al final)
...
ENTROPY_COLS     = {"s1": "AT", "s2": "AK", "s3": "BH", "s4": "AS"}
